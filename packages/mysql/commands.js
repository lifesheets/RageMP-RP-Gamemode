...
mp.events.addCommand('stats', (player) => {
    if (player.admin_level >= 1) {
        ...
        player.outputChatBox(`kein Admin Admin Level: ${player.admin_level}, Money: ${player.data.money} X: ${player.position.x.toFixed(2)} Y: ${player.position.y.toFixed(2)} Z: ${player.position.z.toFixed(2)}`);
    }
});